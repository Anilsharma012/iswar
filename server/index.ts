...
import { loginAdmin, createInitialAdmin, checkSetupStatus, debugAdmins, clearAdmins } from './routes/auth';
import { 
  getClients, 
  getClient, 
  createClient, 
...
app.post('/api/events/:eventId/workers/:workerId/payments', createWorkerPayment);
...
if (process.argv[1] === new URL(import.meta.url).pathname || process.env.NODE_ENV === 'production') {
  connectDb().then(() => {
    app.listen(PORT, () => {
