...
async function upsertClient(name: string, phone: string, extra: Partial<any> = {}) {
...
async function upsertProduct(name: string, sku: string | undefined, category: string, unitType: any, buyPrice: number, sellPrice: number, stockQty: number) {
  const q: any = sku ? { sku } : { name };
  const res = await Product.findOneAndUpdate(
    q,
    { $setOnInsert: { name }, $set: { sku, category, unitType, buyPrice, sellPrice, stockQty } },
    { upsert: true, new: true }
  );
  return res;
}

async function upsertWorker(name: string, phone: string, dailyRate: number, halfDayRate?: number) {
...
async function seed() {
  try {
    ...
    const c1 = await upsertClient('Rahul Sharma', '9876543210', { address: 'Jaipur' });
    const c2 = await upsertClient('Priya Singh', '9876500001', { address: 'Ajmer' });

    await upsertLead('Amit Verma', '9999900001', 'hot');
    await upsertLead('Neha Gupta', '9999900002', 'callback');

    await upsertProduct('Banquet Chair', 'CHAIR-001', 'Seating', 'pcs', 600, 50, 200);
    await upsertProduct('Round Table', 'TABLE-001', 'Tables', 'pcs', 2000, 200, 50);
    await upsertProduct('LED Light', 'LIGHT-LED-01', 'Lighting', 'pcs', 1500, 120, 40);
