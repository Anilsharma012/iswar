...
export const createLead = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await AuditLog.create({ action: 'create', entity: 'Lead', entityId: lead._id, userId: req.adminId ? new mongoose.Types.ObjectId(req.adminId) : undefined, meta: { lead } });
...
export const updateLead = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const lead = await Lead.findByIdAndUpdate(req.params.id, value, { new: true, runValidators: true });
    if (!lead) return res.status(404).json({ error: 'Lead not found' });

    await AuditLog.create({ action: 'update', entity: 'Lead', entityId: lead._id, userId: req.adminId ? new mongoose.Types.ObjectId(req.adminId) : undefined, meta: { lead } });
...
export const deleteLead = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await AuditLog.create({ action: 'delete', entity: 'Lead', entityId: lead._id, userId: req.adminId ? new mongoose.Types.ObjectId(req.adminId) : undefined, meta: { leadId: lead._id } });
...
export const logCall = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await AuditLog.create({ action: 'log-call', entity: 'Lead', entityId: lead._id, userId: req.adminId ? new mongoose.Types.ObjectId(req.adminId) : undefined, meta: { call: value } });
...
export const convertLead = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!client) {
      client = new Client({ name: lead.name, phone: lead.phone, email: lead.email });
    ...
    await AuditLog.create([{ action: 'convert', entity: 'Lead', entityId: lead._id, userId: req.adminId ? new mongoose.Types.ObjectId(req.adminId) : undefined, meta: { clientId: client._id, eventId: event._id } }], { session });
