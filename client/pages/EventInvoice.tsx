...
export default function EventInvoice() {
  ...
  const addManualLine = () => {
    setManualLines((p) => [...p, { productId: "", desc: "", unitType: "pcs", qty: 1, rate: 0 }]);
  };
  ...
  const finalizeAndPDF = async () => {
    try {
      ...
      const inv = res.data;
      // download PDF
      const pdf = await invoiceAPI.downloadPDF(inv._id, inv.language || "en", inv.withGST);
      ...
      <Card>
        ...
        <CardContent>
              ...
              {[...items, ...manualLines].map((row, idx) => (
                <TableRow key={idx}>
                  <TableCell>
                    ...
                    <div className="text-xs text-muted-foreground">{row.unitType}</div>
                  </TableCell>
                  <TableCell>{row.qty}</TableCell>
                  <TableCell>₹{Number(row.rate || 0).toFixed(2)}</TableCell>
                  <TableCell className="font-medium">₹{(Number(row.qty || 0) * Number(row.rate || 0)).toFixed(2)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          ...
          <div className="mt-4">
            ...
            <Input value={String(discount)} onChange={(e) => setDiscount(Number(e.target.value || 0))} />
          </div>
            ...
            <div className="space-y-2">
              {manualLines.map((m, i) => (
                <div key={i} className="flex items-center gap-2">
                  <select value={m.productId} onChange={(e) => updateManual(i, { productId: e.target.value })} className="p-2 border rounded">
                  ...
                  <Input type="number" value={m.qty} onChange={(e) => updateManual(i, { qty: Number(e.target.value) })} className="w-24" />
                  <Input type="number" value={m.rate} onChange={(e) => updateManual(i, { rate: Number(e.target.value) })} className="w-28" />
                  <Button variant="outline" onClick={() => removeManualLine(i)}>Remove</Button>
                </div>
              ))}
              <div>
                <Button variant="outline" onClick={addManualLine}>Add Line</Button>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end mt-4 text-lg font-semibold">
            <div className="text-right">
              ...
              <div>
                ...
                <input type="checkbox" checked={includeSecurity} onChange={(e) => setIncludeSecurity(e.target.checked)} /> {formatINR(securityAmt)}
              </div>
              <div className="mt-2 font-bold">Pending: {formatINR(pending)}</div>
            </div>
          </div>
        </CardContent>
      </Card>
